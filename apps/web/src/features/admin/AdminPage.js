...
export function AdminPage() {
    ...
    useEffect(() => {
        api
            ...
            .then((response) => setWaTemplatesHint(`Log WhatsApp cloud aktif (${Array.isArray(response.data?.data) ? response.data.data.length : 0} pesan)`))
            .catch(() => setWaTemplatesHint("Log WhatsApp belum tersedia"));
    }, []);
    return (_jsx(EnterpriseModulePage, { title: "Platform Governance", subtitle: "Kontrol RBAC, audit trail, approval policy, dan keamanan platform.", points: [
